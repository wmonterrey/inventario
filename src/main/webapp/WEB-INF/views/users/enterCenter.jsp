				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						<li>
							...
							<i class="fa fa-angle-right"></i> <a href="<spring:url value="/users/center" htmlEscape="true "/>"><spring:message code="supercenters" /></a>
						</li>
					</ul>
					...
					<div class="portlet">
						<div class="portlet-title">
							<div class="caption">
								...
								<spring:message code="selectcenter" />
							</div>
							<div class="tools">
						...
						<form action="#" autocomplete="off" id="edit-user-form" class="form-horizontal">
								<div class="form-body">
									...
									<div class="form-group">
										...
										<div class="col-md-5">
											<div class="input-group">
												<input id="completeName" name="completeName" type="text" readonly value="${user.completeName}" class="form-control"/>
												<span class="input-group-addon">
													<i class="fa fa-user"></i>
												</span>
											</div>
									...
									<div class="form-group">
										<label class="control-label col-md-3"><spring:message code="class ni.org.ics.lab.inventario.domain.Center" />:
										<span class="required">
											 *
										</span>
										</label>
										<div class="col-md-5">
											<select data-placeholder="<spring:message code="select" /> <spring:message code="class ni.org.ics.lab.inventario.domain.Center" />" name="userCenter" id="userCenter" class="form-control">
												<option value=""></option>
												<c:forEach items="${centrosusuario}" var="centro">
													<c:choose> 
														<c:when test="${centro.centro.centerCode eq user.userCenter.centerCode}">
															<option selected value="${centro.centro.centerCode}">${centro.centro.centerName}</option>
														</c:when>
														<c:otherwise>
															<option value="${centro.centro.centerCode}">${centro.centro.centerName}</option>
														</c:otherwise>
													</c:choose> 
												</c:forEach>
											</select>
...
<spring:url value="/resources/scripts/users/process-center.js" var="editCenterScript" />
<script src="${editCenterScript}" type="text/javascript"></script>
...
<spring:url value="/users/saveCenter" var="saveCenterUrl"></spring:url>
...
<script>
	jQuery(document).ready(function() {
		...
		var parametros = {saveCenterUrl: "${saveCenterUrl}", homeUrl: "${homeUrl}", successmessage: "${successmessage}",
				errormessage: "${errormessage}"
		};
		EditCenter.init(parametros);
	});
</script>
<!-- END JAVASCRIPTS -->
</body>
